...
# Submit id185/id308 prompt-ablation gen-inv-rec jobs with FPI over seeds 1..10.
#
# Usage:
#   bash scripts/run_id185_308_ablation_fpi_gen_inv_rec.sh
#   bash scripts/run_id185_308_ablation_fpi_gen_inv_rec.sh yagi35 yagi38
#
# Useful overrides:
#   PROMPT_CSV=... RESULTS_DIR=... JOBS_PER_NODE=2 bash scripts/run_id185_308_ablation_fpi_gen_inv_rec.sh yagi35

PROJECT_DIR=${PROJECT_DIR:-$PWD}

TARGET_NODES=${TARGET_NODES:-"yagi29,yagi33,yagi34,yagi35,yagi36,yagi37,yagi38,yagi39,yagi40,yagi41"}
SEEDS=($(seq 1 10))
PROMPT_CSV=${PROMPT_CSV:-results/fpi_gs7_seed_psnr/id185_308_ablation_prompts.csv}
...
OUTPUT_PREFIX=${OUTPUT_PREFIX:-outputs/id185_308_ablation_fpi_gs7_seed}
RESULTS_DIR=${RESULTS_DIR:-results/fpi_gs7_seed_psnr/id185_308_ablation_fpi}
...
if [ "$RUN_AGGREGATE" = "1" ]; then
    ...
    agg_job_id=$(sbatch --parsable \
        ...
        --wrap="bash -c 'source ~/anaconda3/etc/profile.d/conda.sh && conda activate $CONDA_ENV && python analysis/compute_id185_308_ablation_fpi_psnr.py --outputs_dir $AGG_OUTPUTS_DIR --run_prefix $AGG_RUN_PREFIX --results_dir $RESULTS_DIR --seeds 1-10'")
