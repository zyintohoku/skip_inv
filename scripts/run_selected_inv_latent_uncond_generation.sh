...
# Submit unconditional generation from the 20 selected FPI inversion latents.
#
# Usage:
#   bash scripts/run_selected_inv_latent_uncond_generation.sh
#   bash scripts/run_selected_inv_latent_uncond_generation.sh yagi35
#
# Optional env overrides:
#   OUTPUT_DIR=... CONDA_ENV=afpi bash scripts/run_selected_inv_latent_uncond_generation.sh yagi35
#   PARTITION=48-4 bash scripts/run_selected_inv_latent_uncond_generation.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-results/all_prompt_seed_clip_scores/selected_prompt_seed_examples/uncond_from_inv_latents}"
MODEL_NAME="${MODEL_NAME:-CompVis/stable-diffusion-v1-4}"
NUM_DDIM_STEPS="${NUM_DDIM_STEPS:-50}"
GUIDANCE_SCALE="${GUIDANCE_SCALE:-0}"
LOG_DIR="${LOG_DIR:-log}"
JOB_NAME="${JOB_NAME:-selected_inv_uncond}"
PARTITION="${PARTITION:-}"
GPUS="${GPUS:-1}"
