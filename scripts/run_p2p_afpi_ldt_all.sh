...
# Submit P2P editing for AFPI LDT runs with configurable nodes.
# Usage:
#   bash scripts/run_p2p_afpi_ldt_all.sh                        # Auto-select nodes
#   bash scripts/run_p2p_afpi_ldt_all.sh yagi36                # Single node for all jobs
#   bash scripts/run_p2p_afpi_ldt_all.sh yagi36 yagi37 yagi40  # Multiple nodes (cycled)
...
mapfile -t LATENT_DIRS < <(find outputs/reconstruction -maxdepth 1 -type d -name 'afpi_ldt*' | sort -V)

if [ ${#LATENT_DIRS[@]} -eq 0 ]; then
    echo "❌ No latent dirs found under outputs/reconstruction matching afpi_ldt*"
    exit 1
fi
...
for latent_dir in "${LATENT_DIRS[@]}"; do
    ...
    sbatch --job-name="$job_name" \
           ...
           --wrap="bash -c 'source ~/anaconda3/etc/profile.d/conda.sh && conda activate afpi && cd p2p && python p2p.py --latent_dir ../${latent_dir} --output_dir ../${output_dir}'"

    job_idx=$((job_idx + 1))
done

echo ""
echo "✨ All AFPI-LDT P2P jobs submitted!"
echo "Latent source: outputs/reconstruction/afpi_ldt*"
echo "Output target: outputs/editing/afpi_ldt*"
